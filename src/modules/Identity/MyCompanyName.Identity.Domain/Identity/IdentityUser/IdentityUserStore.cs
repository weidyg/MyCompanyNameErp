...
namespace MyCompanyName.Identity
{
      ...
      ITransientDependency
    {
        ...
        public virtual async Task AddToRoleAsync([NotNull] IdentityUser user, [NotNull] string normalizedRoleName, CancellationToken cancellationToken = default)
        {
            ...
            if (await IsInRoleAsync(user, normalizedRoleName, cancellationToken)) { return; }
            var role = await RoleRepository.FindByNormalizedNameAsync(normalizedRoleName, cancellationToken: cancellationToken);
            if (role == null) { throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "Role {0} does not exist!", normalizedRoleName)); }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return roles.Select(r => LookupNormalizer.NormalizeName(r)).Contains(normalizedRoleName);
        }
