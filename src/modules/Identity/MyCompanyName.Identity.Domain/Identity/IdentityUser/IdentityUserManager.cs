...
namespace MyCompanyName.Identity
{
    public class IdentityUserManager : UserManager<IdentityUser>, IDomainService
    {
        ...
        public virtual async Task<IdentityUser> GetByIdAsync(Guid id)
        {
            ...
            if (user == null) { throw new EntityNotFoundException(typeof(IdentityUser), id); }
            return user;
        }
        ...
        {
            await UserRepository.EnsureCollectionLoadedAsync(user, u => u.OrganizationUnits, CancellationTokenProvider.Token);
            if (user.OrganizationUnits.Any(cou => cou.OrganizationUnitId == ou.Id)) { return; }
        ...
        {
            var maxCount = await SettingProvider.GetAsync<int>(IdentitySettings.OrganizationUnit.MaxUserMembershipCount);
            if (requestedCount > maxCount)
            {
                throw new BusinessException(IdentityErrorCodes.MaxAllowedOuMembership).WithData("MaxUserMembershipCount", maxCount);
            }
        }
        ...
        public async Task<bool> ExistPhoneNumberAsync(string phoneNumber, CancellationToken cancellationToken = default)
        {
            if (phoneNumber.IsNullOrWhiteSpace()) { return false; }
            return await UserRepository.AnyAsync(f => f.PhoneNumber == phoneNumber, cancellationToken);
        }
        public async Task<bool> ExistEmailAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            return await UserRepository.AnyAsync(f => f.Email == email, cancellationToken);
        }
    }
}
