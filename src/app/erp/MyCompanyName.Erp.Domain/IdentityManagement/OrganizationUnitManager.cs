...
namespace Sunton.Erp.Identity
{
    public class OrganizationUnitManager : DomainService
    {
        ...
        protected IStringLocalizer<ErpResource> Localizer { get; }
        ...
        public virtual async Task<string> GetNextChildCodeAsync(Guid? parentId)
        {
            ...
            if (lastChild != null) { return OrganizationUnit.CalculateNextCode(lastChild.Code); }
        ...
        {
            if (!recursive) { return await OrganizationUnitRepository.GetChildrenAsync(parentId, includeDetails: true); }
            if (!parentId.HasValue) { return await OrganizationUnitRepository.GetListAsync(includeDetails: true); }
