...
var cardTypeMap = { "DC": "储蓄卡", "CC": "信用卡" };
/*https://jujiewang.oss-cn-hangzhou.aliyuncs.com/Uploads/Jujiewang/Bank/CMBC.png*/
var getBankCardInfo = (cardNo) => {
    var baseurl = `https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardNo=${cardNo}&cardBinCheck=true`;
    return new Promise(function (resolve, reject) {
        $.ajax({
            ...
            success: function (result) {
                if (result && result.stat == "ok" && result.validated == true) {
                    const bankCode = result.bank;
                    const cardTypeCode = result.cardType;
                    const bankName = bankMap[bankCode];
                    const cardType = cardTypeMap[cardTypeCode];
                    resolve({
                        bankCode,
                        bankName,
                        cardTypeCode,
...
const LockoutSet = {
    MaxFailedAccessAttempts: abp.setting.get("Identity.Lockout.MaxFailedAccessAttempts"),
    LockoutDuration: abp.setting.get("Identity.Lockout.LockoutDuration"),
    //Identity.SignIn.EnablePhoneNumberConfirmation: "True"
    //Identity.SignIn.RequireConfirmedEmail: "False"
    //Identity.SignIn.RequireConfirmedPhoneNumber: "False"
};
const UserSet = {
    IsEmailUpdateEnabled: abp.setting.getBoolean("Identity.User.IsEmailUpdateEnabled"),
    IsUserNameUpdateEnabled: abp.setting.getBoolean("Identity.User.IsUserNameUpdateEnabled"),
};
const PasswordRequire = {
    Digit: abp.setting.getBoolean("Identity.Password.RequireDigit"),
    Lowercase: abp.setting.getBoolean("Identity.Password.RequireLowercase"),
    NonAlphanumeric: abp.setting.getBoolean("Identity.Password.RequireNonAlphanumeric"),
    Uppercase: abp.setting.getBoolean("Identity.Password.RequireUppercase"),
    Length: abp.setting.getInt("Identity.Password.RequiredLength"),
};
const $validate = {
    password: (value) => {
        return new Promise(function (resolve, reject) {
            if (value === '') {
                ...
                if (PasswordRequire.Digit && !/\d/.test(value)) {
                    reject($L('Identity:PasswordRequiresDigit'));
                } else if (PasswordRequire.Lowercase && !/[a-z]/.test(value)) {
                    reject($L('Identity:PasswordRequiresLower'));
                } else if (PasswordRequire.Uppercase && !/[A-Z]/.test(value)) {
                    reject($L('Identity:PasswordRequiresUpper'));
                } else if (PasswordRequire.NonAlphanumeric && !/[~!@#$%^&*_]/.test(value)) {
                    reject($L('Identity:PasswordRequiresNonAlphanumeric'));
                } else if (PasswordRequire.Length > 0 && value.Length < PasswordRequire.Length) {
                    reject($L('Identity:PasswordTooShort', PasswordRequire.Length));
                }
                /*else if (abp.setting.getBoolean("Identity.Password.RequiredUniqueChars")) {
        return Promise.reject($L('请输入密码'));
                      }*/
                resolve();
            }
        });
